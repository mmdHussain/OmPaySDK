...
typealias NewVoidRequestCompletionBlock = (_ response:NSDictionary,_ data:Data?,_ error:Any?) -> (Void)
typealias NewVoidRequestCompletionBlock_CreditNonce = (NSDictionary, Data?, Error?) -> Void

struct NewConnectivity {
...
class NewAlamofireHelper: NSObject  {
    static let POST_METHOD = "POST"
    static let GET_METHOD = "GET"
    static let PUT_METHOD = "PUT"
    var new_dataBlock:NewVoidRequestCompletionBlock={_,_,_ in};
    var new_dataBlock_CreditNonce:NewVoidRequestCompletionBlock_CreditNonce={_,_,_ in};
...
//MARK: ---Get Client Token Response---
    func new_getClientTokenResponse(url: String, methodName: String, paramData: String?, block: @escaping NewVoidRequestCompletionBlock) {
        self.new_dataBlock = block
           ...
           /* AF.request(request).responseJSON { response in
                ...
                case .success(_):
                    if let value = response.value as? [String: Any] {
                        self.new_dataBlock(value as NSDictionary, response.data, nil)
                    }
                    break
                ...
                case .failure(_):
                    if let error = response.error {
                        ...
                        self.new_dataBlock(dictResponse as NSDictionary, response.data, error)
                    }
                    break
                }
            }*/
    ...
    //MARK: ---get configuration response---
    func new_ConfigurationResponse(url: String, methodName: String, client_token: String,paramData: String?, block: @escaping NewVoidRequestCompletionBlock) {
        self.new_dataBlock = block
            ...
            AF.request(request).responseJSON { response in
                // Get the status code from the response
                        if let httpURLResponse = response.response {
                            ...
                            if statusCode == 9999 || statusCode == 401 {
...
//MARK: ---GetCreditCardNonce Response---
    func getCreditNonceResponse(url: String, methodName: String, client_token: String ,paramData: String?, block: @escaping voidRequestCompletionBlock_CreditNonce) {
        self.new_dataBlock_CreditNonce = block
        let headers: [String: String] = [
            SDK_HEADERS.ContentType: SDK_HEADERS_VALUES.ContentTypeValue,
            SDK_HEADERS.Authorization: "\(SDK_HEADERS_VALUES.BearerAuthorizationValue)  \(client_token)"
        ]
        ...
        if methodName == AlamofireHelper.POST_METHOD {
            ...
            AF.request(request).responseJSON { response in
                if let httpURLResponse = response.response {
                    let statusCode = httpURLResponse.statusCode
                    print("Status Code of \(url): \(statusCode)")
                ...
                case .success(_):
                    if let value = response.value as? [String: Any] {
                        self.new_dataBlock_CreditNonce(value as NSDictionary  ,response.data,nil)
                    }
                case .failure(_):
                    if let error = response.error {
                        let dictResponse: [String: Any] = [:]
                        self.new_dataBlock_CreditNonce(dictResponse as NSDictionary ,response.data,nil)
                    }
                }
            }
        }
